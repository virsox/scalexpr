...
trait Variable[T] extends SimpleExpression[T] {
  ...
  override def resolve[A](context: A = EmptyContext)(implicit ctxLike: ContextLike[A]): T = {
    ...
    val lastEventOption = eventNames.foldLeft(Some(context): Option[A])(
      (ctx, field) => ctx.flatMap((c) => ctxLike.nested(c, field))
    )

    lastEventOption match {
      case Some(ctx) => ctxLike.get[T](ctx, attribute(0)) match {
